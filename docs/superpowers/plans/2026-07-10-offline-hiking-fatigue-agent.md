...
- [ ] 添加 XCTest target 和脱敏 GPX fixture。
- [ ] 先写解析、跨夜间隔、时间倒退、重复点和统计的失败测试。
- [ ] 实现容错 GPX 解析和派生统计，区分计划与历史用途。
- [ ] 运行测试/构建验证，提交并推送。
...
- [ ] 先写数据新鲜度和准备度测试。
- [ ] 实现徒步相关 HealthKit 类型、主动授权、查询和 observer。
- [ ] 实现问卷补缺、来源/时间戳/可信度模型。
- [ ] 将 HealthKit、问卷和 GPX 变成连续聊天内卡片。
- [ ] 验证权限拒绝/无数据路径，提交并推送。
...
- [ ] 先写路线负荷、挑战差距、补给、装备和行动白名单测试。
- [ ] 实现十二个结构化规则工具与固定模板回退。
- [ ] 接入 Qwen3 原生 tool call，校验参数和工具结果。
- [ ] 在聊天流展示三分报告、补给和装备卡片。
- [ ] 验证模型不可用时完整可用，提交并推送。
...
- [ ] 先写路线匹配、风险叠加、升级和冷却测试。
- [ ] 实现前后台定位、实际轨迹记录和最近路线进度。
- [ ] 实现事件驱动评估与前台 30 秒节流，不使用伪实时模拟。
- [ ] 实现本地通知、固定安全模板和主观状态确认。
- [ ] 验证定位/通知拒绝与数据过旧路径，提交并推送。
...
- [ ] 移除生产流程中的固定 SampleData 和 1 秒模拟依赖。
- [ ] 配置 HealthKit、定位、后台定位和通知描述/entitlements。
- [ ] 走通首页→Stage 1→门禁→Stage 2→Stage 3→首页。
- [ ] 检查所有页面无顶部白条且只使用现有颜色与组件。
- [ ] 运行全量测试、真机构建和静态检查，提交并推送。
